  ...
  function($scope, enumResource) {
    enumResource.getAll($scope.model.config.assembly, $scope.model.config.enum).then(function(response) {
      ...
      function setupViewModel() {
        ...
        // now we need to check if the value is null/undefined, if it is we need to set it
        // to "" so that any value that is set to "" gets selected by default
        if ($scope.model.value === null || $scope.model.value === undefined) {
          $scope.model.value = [];
        }

        for (const i in $scope.enumItems) {
          const isChecked = _.contains($scope.model.value, $scope.enumItems[i]);
      ...
      $scope.$watch('selectedItems',
        function(newVal, oldVal) {
          $scope.model.value = [];
          for (let x = 0; x < $scope.selectedItems.length; x++) {
